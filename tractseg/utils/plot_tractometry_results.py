    ...
    with open(file_path) as f:
        ...
        for i in range(2):
            ...
            if l.startswith("# bundles="):
                ...
                valid_bundles = dataset_specific_utils.get_classes("tractometry50")[1:]
        ...
        if bundles is None:
            bundles = dataset_specific_utils.get_classes("tractometry50")[1:]
    ...
    for b_idx in selected_bun_indices:
        for jdx in range(NR_POINTS):
            ...
            if analysis_type == "group":
                target_cor = metric_utils.unconfound(target, meta_data[["group"] + confound_names].values, group_data=True).squeeze()
            else:
                target_cor = metric_utils.unconfound(target, meta_data[confound_names].values, group_data=False).squeeze()
                meta_data["target"] = metric_utils.unconfound(
                    meta_data["target"].values[..., None], meta_data[confound_names].values, group_data=False
                ).squeeze()
...
def get_corrected_alpha(values_allp, meta_data, analysis_type, subjects_A, subjects_B, alpha, bundles, nperm, b_idx):
    ...
    alphaFWE, statFWE, clusterFWE, stats = AFQ_MultiCompCorrection(np.array(values_allp), y, alpha, nperm=nperm)
...
def plot_tractometry_with_pvalue(
    values,
    meta_data,
    bundles,
    selected_bundles,
    ...
    values = correct_for_confounds(values, meta_data, bundles, selected_bun_indices, NR_POINTS, analysis_type, confound_names)
    ...
    for i, b_idx in enumerate(tqdm(selected_bun_indices)):
        ...
        data = {"position": [], "fa": [], "group": [], "subject": []}
        ...
        if FWE_method == "alphaFWE":
            axes[i].annotate(
                "alphaFWE:   {}".format(format_number(alphaFWE)),
                (0, 0),
                (0, -35),
            ...
            axes[i].annotate(
                "min p-value: {}".format(format_number(pvalues.min())),
                (0, 0),
                (0, -45),
                xycoords="axes fraction",
                textcoords="offset points",
            ...
            axes[i].annotate(
                "clusterFWE:   {}".format(clusterFWE), (0, 0), (0, -35), xycoords="axes fraction", textcoords="offset points", va="top", fontsize=10
            )
            results_df.at[i, "clusterFWE"] = clusterFWE

        stats_label = "t-value:      " if analysis_type == "group" else "corr.coeff.: "
        axes[i].annotate(
            stats_label + "   {}".format(format_number(stats[pvalues.argmin()])),
            (0, 0),
            (0, -55),
            xycoords="axes fraction",
            textcoords="offset points",
        ...
        if plot_3D_type != "none":
            ...
            plot_utils.plot_bundles_with_metric(
                tracking_path, ending_path, mask_path, bundle, metric, output_path_3D, tracking_format, show_color_bar
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test for significant differences and plot tractometry results.", epilog="Written by Jakob Wasserthal."
    )
    parser.add_argument("-i", metavar="subjects_file_path", dest="subjects_file", help="txt file containing path of subjects", required=True)
    parser.add_argument("-o", metavar="plot_path", dest="output_path", help="output png file containing plots", required=True)
    parser.add_argument("--mc", action="store_true", help="correct for multiple tract comparison", default=False)
    parser.add_argument("--nperm", metavar="n", type=int, help="Number of permutations (default: 5000)", default=5000)
    parser.add_argument("--alpha", metavar="a", type=float, help="The desired alpha level (default: 0.05)", default=0.05)
    parser.add_argument(
        "--plot3D",
        metavar="none|metric|pval",
        choices=["none", "metric", "pval"],
        help="Generate streamline plots of each tract with FA ('metric') or significant areas " "('pval') on top",
        default="none",
    )
    parser.add_argument("--save_csv", action="store_true", help="save results also as csv", default=False)
    parser.add_argument(
        "--tracking_dir",
        metavar="folder_name",
    ...
    parser.add_argument(
        "--tracking_format",
        metavar="tck|trk|trk_legacy",
        choices=["tck", "trk", "trk_legacy"],
        help="If using --plot3D you have to specify the format of the trackings which will get loaded." "(default: tck)",
        default="tck",
    )
    parser.add_argument(
        "--range",
        ...
        default=None,
        type=two_floats,
        help="Range of metric (y-axis) to plot. Specify lower (l) and upper (u) bound" "(default: None)",
    )
    ...
    if meta_data.columns[1] == "group":
        ...
        nperm = int(nperm / 5)
        print("Using one fifth ({}) of the number of permutations for correlation analysis " "because this has longer runtime".format(nperm))
        print("Number of subjects: {}".format(len(meta_data)))
    else:
        raise ValueError("Invalid second column header (only 'group' or 'target' allowed)")

    all_bundles = dataset_specific_utils.get_classes("tractometry50")[1:]
