...
def reorient_to_std_space(input_file, bvals, bvecs, brain_mask, output_dir):
    ...
    os.system("rotate_bvecs -i " + bvecs + " -t " + output_dir + "/reorient2std.mat -o " + output_dir + "/Diffusion_MNI.bvecs")

    os.system(
        "flirt -ref "
        + output_dir
        + "/Diffusion_MNI.nii.gz -in "
        + brain_mask
        + " -out "
        + output_dir
        + "/nodif_brain_mask_MNI.nii.gz -applyxfm -init "
        + output_dir
        + "/reorient2std.mat -dof 6"
    )
...
def move_to_MNI_space(input_file, bvals, bvecs, brain_mask, output_dir):
    ...
    os.system("calc_FA -i " + input_file + " -o " + output_dir + "/FA.nii.gz --bvals " + bvals + " --bvecs " + bvecs + " --brain_mask " + brain_mask)

    dwi_spacing = img_utils.get_image_spacing(input_file)

    template_path = importlib_resources.files("tractseg.resources") / "MNI_FA_template.nii.gz"
    ...
    os.system(
        ...
        + " -out "
        + output_dir
        + "/Diffusion_MNI.nii.gz -applyisoxfm "
        + dwi_spacing
        + " -init "
        + output_dir
        + "/FA_2_MNI.mat -dof 6 -interp spline"
    ...
    os.system("cp " + bvals + " " + output_dir + "/Diffusion_MNI.bvals")
    os.system("rotate_bvecs -i " + bvecs + " -t " + output_dir + "/FA_2_MNI.mat" + " -o " + output_dir + "/Diffusion_MNI.bvecs")
    ...
    os.system(
        ...
        + " -out "
        + output_dir
        + "/nodif_brain_mask_MNI.nii.gz -applyisoxfm "
        + dwi_spacing
        + " -init "
        + output_dir
        + "/FA_2_MNI.mat -dof 6 -interp nearestneighbour"
    )
