...
def get_bundle_names(CLASSES):
    if CLASSES == "All":
        ...
        # 11 Major tracts
        bundles = ["CST_left", "CST_right", "IFO_left", "IFO_right", "CA", "CG_left", "CG_right", "FX_left", "FX_right", "UF_left", "UF_right"]
...
def set_labels_filename():
    ...
    elif (config.CLASSES == "AutoPTX" or config.CLASSES == "AutoPTX_42") and config.EXPERIMENT_TYPE == "tract_segmentation":
        if config.RESOLUTION == "1.25mm":
    ...
    elif (config.CLASSES == "AutoPTX" or config.CLASSES == "AutoPTX_42") and config.EXPERIMENT_TYPE == "dm_regression":
        if config.RESOLUTION == "1.25mm":
...
def get_dwi_affine(dataset, resolution):
    if dataset == "HCP" and resolution == "1.25mm":
        # shape (145,174,145)
        return np.array([[-1.25, 0.0, 0.0, 90.0], [0.0, 1.25, 0.0, -126.0], [0.0, 0.0, 1.25, -72.0], [0.0, 0.0, 0.0, 1.0]])
        ...
        # shape (90,108,90)
        return np.array([[-2.0, 0.0, 0.0, 90.0], [0.0, 2.0, 0.0, -126.0], [0.0, 0.0, 2.0, -72.0], [0.0, 0.0, 0.0, 1.0]])

    elif (dataset == "HCP" or dataset == "HCP_32g" or dataset == "HCP_2.5mm") and resolution == "2.5mm":
        # shape (73,87,73)
        return np.array([[-2.5, 0.0, 0.0, 90.0], [0.0, 2.5, 0.0, -126.0], [0.0, 0.0, 2.5, -72.0], [0.0, 0.0, 0.0, 1.0]])
...
def scale_input_to_original_shape(img4d, dataset, resolution="1.25mm"):
    ...
    if resolution == "1.25mm":
        if dataset == "HCP":  # (144,144,144)
            # no resize needed
            return img_utils.pad_4d_image_left(img4d, np.array([1, 15, 1, 0]), [146, 174, 146, img4d.shape[3]], pad_value=0)[
                :-1, :, :-1, :
            ]  # (145, 174, 145, none)
