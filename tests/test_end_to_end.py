...
def transform_to_output_space(data):
    transformation = {"original_shape": (57, 70, 59, 9), "pad_x": 6.5, "pad_y": 0.0, "pad_z": 5.5, "zoom": 2.057142857142857}
...
class test_end_to_end(unittest.TestCase):
    ...
    def test_csd_peaks(self):
        ...
        img_new = np.nan_to_num(nib.load("resources/examples/docker_test/peaks.nii.gz").get_fdata())
    ...
    def test_bundle_specific_postprocessing(self):
        ...
        img_new = nib.load("resources/examples/BS_PP/tractseg_output/bundle_segmentations/CA.nii.gz").get_fdata().astype(np.uint8)
        images_equal = np.array_equal(img_ref, img_new)
        ...
        img_ref[10:30, 10:30, 40:50] = 1  # big blob 2
        img_ref = transform_to_output_space(img_ref)
        img_new = nib.load("resources/examples/BS_PP/tractseg_output/bundle_segmentations/CC_1.nii.gz").get_fdata().astype(np.uint8)
        images_equal = np.array_equal(img_ref, img_new)
    ...
    def test_postprocessing(self):
        ...
        img_ref = transform_to_output_space(img_ref)
        img_new = nib.load("resources/examples/no_PP/tractseg_output/bundle_segmentations/CC_1.nii.gz").get_fdata().astype(np.uint8)
        images_equal = np.array_equal(img_ref, img_new)
        self.assertTrue(images_equal, "Tract segmentations are not correct (bundle: CC_1)")
    ...
    def test_get_probabilities(self):
        ...
        img_ref = transform_to_output_space(img_ref)
        img_new = nib.load("resources/examples/Probs/tractseg_output/bundle_segmentations/CA.nii.gz").get_fdata()
        images_equal = np.array_equal(img_ref, img_new)
    ...
    def test_uncertainty(self):
        ...
        img_ref[60:63, 60:63, 60:63] = 0.9  # small blob -> will get removed by postprocessing
        img_ref = transform_to_output_space(img_ref)
        img_new = nib.load("resources/examples/Uncert/tractseg_output/bundle_uncertainties/CA.nii.gz").get_fdata()
        images_equal = np.array_equal(img_ref, img_new)
    ...
    def test_density_regression(self):
        ...
        img_ref[60:63, 60:63, 60:63] = 0.9  # small blob -> will get removed by postprocessing
        img_ref = transform_to_output_space(img_ref)
        img_new = nib.load("resources/examples/DM/tractseg_output/dm_regression/CA.nii.gz").get_fdata()
        images_equal = np.array_equal(img_ref, img_new)
    ...
    def test_tractseg_output_SR_noPP(self):
        ...
        for bundle in bundles:
            ...
            if bundle != "IFO_right":
                ...
                img_new = (
                    nib.load("resources/examples/SR_noPP/tractseg_output/bundle_segmentations/" + bundle + ".nii.gz").get_fdata().astype(np.uint8)
                )
                ...
                self.assertTrue(
                    images_equal,
                    "Tract segmentations are not correct (bundle: " + bundle + ") " + "(nr of differing voxels: " + str(nr_differing_voxels) + ")",
                )
    ...
    def test_endingsseg_output(self):
        ...
        for bundle in bundles:
            ...
            img_new = nib.load("resources/examples/tractseg_output/endings_segmentations/" + bundle + "_e.nii.gz").get_fdata().astype(np.uint8)
            images_equal = np.array_equal(img_ref, img_new)
    ...
    def test_tractometry_toy_example(self):
        ref = np.array([0.0, 0.0, 0.148, 0.173, 0.0, 0.325, 0.319, 0.28])  # coord + tree, round(3)
    ...
    def test_tractometry(self):
        ...
        new = np.loadtxt("resources/examples/Tractometry.csv", delimiter=";", skiprows=1).transpose()
    ...
    def test_statistical_analysis_group(self):
        ref = (
            pd.read_csv("tests/reference_files/tractometry/tractometry_result_group.png.csv", sep=",").values[:, 2:].astype(np.float32)
        )  # all but index and bundle_name column
        new = pd.read_csv("resources/examples/tractometry_result_group.png.csv", sep=",").values[:, 2:].astype(np.float32)
        diff_max = np.abs(ref - new).max()
        arrays_equal = np.array_equal(ref, new)
        self.assertTrue(arrays_equal, "Statistical analysis (group) not correct (max difference: " + str(diff_max) + ")")

    def test_statistical_analysis_correlation(self):
        ref = (
            pd.read_csv("tests/reference_files/tractometry/tractometry_result_correlation.png.csv", sep=",").values[:, 2:].astype(np.float32)
        )  # all but index and bundle_name column
        new = pd.read_csv("resources/examples/tractometry_result_correlation.png.csv", sep=",").values[:, 2:].astype(np.float32)
        diff_max = np.abs(ref - new).max()
        ...
        self.assertTrue(arrays_equal, "Statistical analysis (correlation) not correct (max difference: " + str(diff_max) + ")")
